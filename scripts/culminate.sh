...
curl --fail --silent --show-error --request POST --output /dev/null \
  ...
  --data "{ \"request\": { \"branch\": \"developer\", \"message\": \"Build to parity with commit "$TRAVIS_COMMIT" on "$TRAVIS_REPO_SLUG"\" } }" \
  --url "https://api.travis-ci.org/repo/ashenm%2Fbaseimage/requests"
